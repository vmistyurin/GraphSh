...
namespace GraphSh
{
    public sealed class SymmetricMatrixGraph : GraphBase
    {
        private SymmetricMatrix _matrix;

        public override int Dimension => _matrix.Dimension;

        public SymmetricMatrixGraph(int[,] matrix) : base(matrix)
        {
            _matrix = new SymmetricMatrix(matrix);
        }

        public SymmetricMatrixGraph(int dimension, List<Edge> edges) : base(dimension, edges)
        {
            _matrix = new SymmetricMatrix(dimension);
            foreach (var e in edges)
            {
                _matrix[e.FirstV, e.SecondV] = 1;
            }
        }

        public override object Clone()
        {
            return new SymmetricMatrixGraph(_matrix.GetMatrix());
        }
        ...
        public override List<int> DeleteVertexes(params int[] numbers)
        {
            ...
            SymmetricMatrix newMatrix = new SymmetricMatrix(newDimension);
        ...
        {
            SymmetricMatrix newMatrix = new SymmetricMatrix(Dimension);
            for (int i = 0; i < Dimension; i++)
                for (int j = 0; j < Dimension; j++)
                {
                    if (_matrix[i, j] == 1)
        ...
        public override List<int> MergeVertexes(List<List<int>> mergedList, List<bool> isImportant)
        {
            ...
            for (int i = 0; i < Dimension; i++)
            {
                ...
                if (newPoint == -1)
                {
                    ...
                    {
                        if (i < freeNumbers.Peek())
                            newNumber = i;
                        else
                            newNumber = freeNumbers.Dequeue();
                    }
                ...
                else
                {
                    for (int j = 0; j < mergedList[newPoint].Count; j++)
                        newNumbers[mergedList[newPoint][j]] = mergedList[newPoint][0];
                    for (int j = 1; j < mergedList[newPoint].Count; j++)
                        freeNumbers.Enqueue(mergedList[newPoint][j]);
                    mergedPoints[mergedPointsCount] = mergedList[newPoint];
                }
            }

            SymmetricMatrix newMatrix = new SymmetricMatrix(newSize);
